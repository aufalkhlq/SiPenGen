        ...
        for ($i = 0; $i < $jumlahIndividu; $i++) {
            ...
            foreach ($pengampus as $pengampu) {
                ...
                for ($s = 0; $s < $totalJadwal; $s++) {
                    ...
                    do {
                        $jadwal = $this->buatJadwal($jumlahHari, $jumlahJam, $jumlahRuangan, $kelas_id, $pengampu->id, $matkul->nama, $jadwalMap, $kelasJamPerHari[$kelas_id]);
                        $attempts++;
        ...
        for ($hari_id = 1; $hari_id <= $jumlahHari; $hari_id++) {
            ...
            for ($jam_id = 1; $jam_id <= $jumlahJam; $jam_id++) {
                ...
                for ($ruangan_id = 1; $ruangan_id <= $jumlahRuangan; $ruangan_id++) {
                    // Ensure that the matkul jaringan is placed in the network lab
                    if ($matkulNama == 'Jaringan' && !Ruangan::find($ruangan_id)->isLabJaringan()) {
                        continue; // Skip if it's not the network lab
                    }
    ...
    public function mutasi($individu, $jumlahJam, $jumlahHari, $jumlahRuangan) {
        ...
        foreach ($individu as &$jadwal) {
            if (rand(0, 100) / 100 < $probabilitasMutasi) {
                ...
                $matkulNama = Pengampu::find($pengampu_id)->matkul->nama;
